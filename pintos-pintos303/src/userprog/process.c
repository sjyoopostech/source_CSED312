...
start_process(void *pcb_)
{
    ...
    /*Project3 Edited*/
    vmem_init(&(thread_current()->hash_table));
...
void process_exit(void)
{
    ...
    for (e = list_begin(&cur->mmap_list); e != list_end(&cur->mmap_list); ) {
        struct file_mmap *mfile = list_entry(e, struct file_mmap, elem);
        e = list_next(e);
        munmap_file(mfile);
    }

    vm_remove_all(&(cur->hash_table));
             ...
             uint32_t read_bytes, uint32_t zero_bytes, bool writable)
{
    ...
    while (read_bytes > 0 || zero_bytes > 0)
    {
        ...
        struct vm_entry* page = alloc_vm(upage,writable);
...
setup_stack(void **esp)
{
    ...
    struct lru_page *p = alloc_page(PAL_USER | PAL_ZERO);
    
    if (p->paddr != NULL)
    {
        success = install_page(((uint8_t *)PHYS_BASE) - PGSIZE, p->paddr, true);
        if (success)
            *esp = PHYS_BASE;
        else
            free_page(p->paddr);
    ...
    if (success) {
        struct vm_entry* page = alloc_vm(((uint8_t*)PHYS_BASE) - PGSIZE, true);
        p->vm_entry = page;
        if (page == NULL) {
            success = false;
        }
        else {
            if (load_entry(page, p->paddr)) {
                success = true;
...
bool handle_fault(struct vm_entry* e) {
    if (e != NULL) {
        enum palloc_flags user_flag = PAL_USER;
       
        if (e->read_bytes == 0) {
            user_flag = user_flag | PAL_ZERO;
        }
        struct lru_page *p = alloc_page(user_flag);
        p->vm_entry = e;
        ...
        {
        case VM_BIN:
            if (!load_entry(e,p->paddr)) {
                //printf("load_entry failed\n");
                return false;
            }
            
            if (!install_page(e->vaddr, p->paddr, e->writable)){
            ...
            return true;
            break;
        //other types are TODOs for mmf and swap
        case VM_FILE:
            if (!load_mmap_file(e, p->paddr)) {
                //printf("load_entry failed\n");
                return false;
            }
...
bool expand_stack(void * addr) {
    enum palloc_flags user_flag = PAL_USER;
    void* kpage = palloc_get_page(user_flag | PAL_ZERO);
    bool success = false;
    
    if (kpage != NULL && install_page(pg_round_down(addr), kpage, true)) {
        struct vm_entry* page = alloc_vm(pg_round_down(addr), true);
            if (page != NULL) {
