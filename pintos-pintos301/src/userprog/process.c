             ...
             uint32_t read_bytes, uint32_t zero_bytes, bool writable)
{
    ...
    while (read_bytes > 0 || zero_bytes > 0)
    {
        ...
        struct vm_entry* page = alloc_page(upage,writable);
        if (page == NULL)
            return false;
...
setup_stack(void **esp)
{
    ...
    if (success) {
        struct vm_entry* page = alloc_page(((uint8_t*)PHYS_BASE) - PGSIZE, true);
        if (page == NULL) {
            success = false;
        }
        else {
            ...
            void* buffer = palloc_get_page(user_flag);
            if (load_entry(page, buffer)) {
                success = true;
...
bool handle_fault(struct vm_entry* e) {
    if (e != NULL) {
        ...
        {
        case VM_BIN:
            if (!load_entry(e,buffer)) {
                //printf("load_entry failed\n");
                return false;
            }
            
            if (!install_page(e->vaddr, buffer, e->writable)){
