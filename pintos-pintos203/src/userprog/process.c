...
process_execute (const char *file_name)
{
  ...
  if (filesys_open(token) == NULL) {
      filesys_remove(token);
      palloc_free_page(fn_copy);
      free(token);
      return -1;
  }
  /* Project2 Edited */
  /* Parsing file name from arguments */
  ...
  for (struct list_elem* iter = list_begin(&cur->child_list); iter != list_end(&cur->child_list); iter = list_next(iter)) {
...
void argument_load(char **esp, char **words, int cnt) {
...
start_process (void *file_name_)
{
  ...
  success = load (words[0], &if_.eip, &if_.esp);
  ...
  /* Project2 Edited */
  if (success) {
      argument_load((char**)(&if_.esp), words, cnt);
      t->loaded = true;
      //hex_dump((uintptr_t) if_.esp, if_.esp, PHYS_BASE-if_.esp, true); 
  }
  palloc_free_page(words);
  /* If load failed, quit. */
...
{
    /* Project2 Edited */
    /* waiting for termination of child process */
    struct thread* child = get_child(child_tid);
    if (child != NULL) {
        sema_down(&(child->sema_exit));
        int stat = child->exit_status;
...
process_exit (void)
{
  ...
  /* Project2 Edited */
  /* file execute */
  //file_close(cur->filex);
  //cur->filex = NULL;

  /* Project 2 Edited */
  /* file descriptor */
  close(-1);
  //sema_down(&(cur->sema_file));
  /*
...
struct thread* get_child(tid_t pid) {
    ...
    for (i = list_begin(children); i != list_end(children); i = list_next(i)) {
        //get element of list_elem i
        struct thread* temp = list_entry(i, struct thread, child_elem);
        //if tid of thread is same with pid then return
        if (temp->tid == pid) {
            return temp;
        }
    }
